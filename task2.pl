% Пример представления данных из four.pl
...
group_avg_grade(Group, AvgGrade) :-
    findall(Grade, (subject(_, Grades), member(grade(Student, Grade), Grades), group(Group, Students), member(Student, Students)), AllGrades),
    length(AllGrades, N),
    sum_list(AllGrades, Sum),
    AvgGrade is Sum / N.

% Для каждого предмета получить список студентов, не сдавших экзамен (grade=2)
failed_students_by_subject(Subject, Students) :-
    findall(Student, (subject(Subject, Grades), member(grade(Student, 2), Grades)), Students).

% Найти количество не сдавших студентов в каждой из групп
count_failed_students_by_group(Group, Count) :-
    findall(Student, (subject(_, Grades), member(grade(Student, 2), Grades), group(Group, Students), member(Student, Students)), FailedStudents),
